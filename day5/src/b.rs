...
fn find_word(lines: &Vec<String>, word: &str) -> u32 {
    ...
    for y in 0..h {
        for x in 0..w {
            ...
            //print!("{c}");
            mark_words(lines, word, x, y, &mut state, "ne");
            mark_words(lines, word, x, y, &mut state, "sw");
            mark_words(lines, word, x, y, &mut state, "nw");
            mark_words(lines, word, x, y, &mut state, "se");
        }
        //println!("")
    }

    let many = |d: &HashMap<&str, bool>| {
        let mut sum: u32 = 0;
